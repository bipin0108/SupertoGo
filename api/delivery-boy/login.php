...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
	...
	$query=$con->prepare("SELECT * FROM `tbl_delivery_boy` WHERE email=:email and password=:password");
	...
	if($query->rowCount()>0){
		...
		if($result['status'] == 'Active'){
			...
			$id = updateRow("tbl_delivery_boy",$data,array('db_id'=>$result['db_id']));
			$results =  getRow("SELECT * FROM tbl_delivery_boy WHERE db_id=:db_id",array("db_id"=>$result['db_id']));
